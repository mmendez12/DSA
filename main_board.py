...
class Game:
    ...
    @property
    def n_player_back(self):
        return len([p for p in self.players if p.is_back])
    ...
    def start_next_round(self):
        # the furthest player plays first
        # TODO: keep track of the order in which player came back
        ...
        self.board.tile_status = [0] * len(no_empty_road)
        self.round += 1
...
def main():
    ...
    while True:
        ...
        if all(player.is_back for player in game.players) or game.round_end:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if (event.key == pg.K_r) and game.can_roll:
                    ...
                    print(game.current_player, f" roll: {dice_roll}")
                    game.submarine.air -= game.current_player.n_bag
                    print(game.submarine)
                    if game.current_player.is_back:
                        # TODO: make a function?
                        # deduct 1 to make sure that that player.position < 0 (check player.is_back)
                        game.current_player.position = game.n_player_back - N_PLAYER - 1
                        game.current_player.move_to_submarine()
                    else:
                        x, y = board.road[game.current_player.position].rect.center
                    ...
                    if level_selected not in levels:
                        break
                    else:
